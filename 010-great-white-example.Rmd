...
"second-born" a lot to describe these to members of each pair. 
...
be thought of as separate "chances" to be a half sibling.  
...
population size always depends on the number of adults _at time of the
second-born's birth_, and not at the time of the first-born's birth.

The foregoing were the calculations for being a maternal half sibling, but
each pair also has a separate chance of being a paternal half sibling, which
is the same in this model used by @hillary2018genetic.  Since the events
...
might be involved in the derivative of the function must be defined as a `Type`
variable.  We will see what this means as we go through the code.

The following code is stored in a file `TMB/hsp_nll.cpp`, relative to the
current working directory, that is already in this repository.
```c
...
- There is always some boilerplate that loads libraries and tells 'TMB'
...
PARAMETER(phiA); 
    ```
Each one of those is a scalar.  `N_init` here is `N0`. 
...
Type nll = 0; 
    ```
- All that remains then is calculating and returning the objective function.  In our case
...
And then, in order to get our data into it that function we defined
...
For unknown reasons,on a Mac, 'TMB' seems
...
Additionally, at least on a Mac, 'TMB' seems to do a poor job of realizing whether
...
Now that we have a fast way to evaluate the negative log likelihood using TMB, let's
