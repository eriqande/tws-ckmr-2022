...
After that we will make some data and parameter lists:
```{r}
...
Now, to make an AD function with TMB, we need to link to the library
created when we compiled the file above.  For unknown reasons,on a Mac, TMB seems
...
if(Sys.info()["sysname"] == "Darwin") {
  ...
  obj <- TMB::MakeADFun(data = data, parameters = parameters, DLL="TMB/hsp_nll")
}
```
...
to visualize:
```{r contour, fig.width=6, out.width='100%',cache=TRUE}
LL_tib %>%
  group_by(lambda) %>%
